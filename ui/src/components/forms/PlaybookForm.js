...
const PlaybookForm = (props) => {
    const { form, submitLabel, onSubmit } = props
    const [name, setName] = useState(form?.name || '');
    const [project_id, setProjectId] = useState(form?.project_id || null);
    const [json_playbook, setJsonPlaybook] = useState(form?.json_playbook || '');

    const handleSubmit = (e) => {
        e.preventDefault();

        const payload = { name, project_id, json_playbook }
        onSubmit(payload)
    }
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="w-full mb-5">
                    ...
                    <input
                        className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:text-gray-600"
                        value={json_playbook}
                        onChange={(e) => setJsonPlaybook(e.target.value)}
                        type="text"
                ...
                <div className="text-center mt-4 text-gray-500"><Link to='/playbooks'>Cancel</Link></div>
            </form>
        </div>
    )
}
